...
treeview <- function(e0,
	...
	branch_cols <- unique(c("logistic_growth_rate",
	                        branch_cols))
	...
	cmuts <- lapply(seq_len(nrow(sc0)), function(i) {
		list(defining = strsplit(sc0$all_mutations[i], split = "\\|")[[1]],
	...
	for (i in seq_along(stres2)){
		...
		uv <- stats::na.omit(sc0$node_number[match(stres2[[i]]$tip.label,
		                                    sc0$representative)])
		shared_anc <- Reduce(intersect,
		                     e0$ancestors[uv])
		shared_anc2 <- setdiff(intersect(shared_anc,
		                                 sc0$node_number),
		                       uv)
	...
	sc0$tr2mrca[i] <- match(sc0$representative[i],
	                        tr2$tip.label)

	# tree data frames
	## tips
	sc2 <- sc0[!is.na(sc0$tr2mrca), ]
	sc2$date_range <- sapply(seq_len(nrow(sc2)),
	                         function(i) glue::glue("{sc2$least_recent_tip[i]} -> {sc2$most_recent_tip[i]}"))
	tdvars <- unique(c(branch_cols,
	...
	if (nrow(td1) > 0) {
		...
		td1$internal <- "Y"
		td1$cluster_size <- 0
		x <- setdiff((ape::Ntip(tr2) + 1):(ape::Ntip(tr2) + ape::Nnode(tr2)),
		             td1$node) # make sure every node represented
		td1 <- merge(td1,
		             data.frame(node = x),
		             all = TRUE)
		td <- rbind(td0, td1)
	...
	.plot_tree <- function(vn,
		...
		shapes <- c(Y = "\U2B24",
		            N = "\U25C4")
		gtr1.1 <- gtr1 +
		  ggplot2::scale_color_gradientn(name = gsub(vn, pattern = "_", replacement = " "),
		                                 colours = cols,
		                                 limits = colour_limits,
		                                 oob = scales::squish) +
		  ggplot2::geom_point(ggplot2::aes_string(color = vn,
		                                          size = "cluster_size",
		  ...
		  ggplot2::scale_shape_manual(name = NULL,
		                              labels = NULL,
		                              values = shapes) +
		  ggplot2::scale_size(name = "Cluster size",
		                      range = c(2, 16)) +
		...
		for (i in  seq_along(lins)) {
			if (!is.na(lin_nodes[i])) {
				gtr1.1 <- gtr1.1 +
				  ggtree::geom_cladelabel(node = lin_nodes[i],
		...
		ttdfs <- apply(gtr1.1$data, 1, FUN = function(x) {
			...
			y <- with(z,
			         data.frame(
			           ...
			           `Logistic growth` = paste0(ifelse(lgr > 0, "+", ""),
			                                      round(lgr * 100), "%"),
		...
		genotype <- as.data.frame(gtr1.1$data[gtr1.1$data$node <= ape::Ntip(tr2),
		                                      c("label", mut_regex)])
		...
		gtr1.3 <- gtr1.2 +
		ggiraph::geom_point_interactive(ggplot2::aes(x = .data$x,
		                                             y = .data$y,
		  ...
		  ggplot2::scale_size(name = "Cluster size",
		                      range = c(2, 16)) +
		  ggplot2::scale_color_gradientn(name = stringr::str_to_title(gsub(vn,
		                                                                   pattern = "_",
		                                                                   replacement = " ")),
		                                 colours  = cols,
		                                 limits = colour_limits,
		...
		pgtr1.3 <- ggiraph::girafe(
		  ...
		  options = list(ggiraph::opts_sizing(width = 0.8),
