...
treeview <- function(e0,
  ...
  .plot_tree <- function(vn, mut_regex = NULL, colour_limits = NULL) {
    ...
    ggsave(gtr1.1,
      file = glue("{output_dir}/tree-{vn}-{Sys.Date()}.svg"),
    ...
    gtr1.3 <- gtr1.2 +
      ...
      scale_size(name = "Cluster size", range = c(2, 16)) +
      scale_color_gradientn(
        name = stringr::str_to_title(gsub(vn, patt = "_", rep = " ")),
        colours = cols,
        limits = colour_limits,
        oob = scales::squish
      ) +
  ...
  .pl_cluster_sina <- function(pldf, varx = "logistic_growth_rate",
    ...
    p1 <- ggplot(sc1) +
      ggiraph::geom_point_interactive(aes(x = x, y = y, colour = mutation_lineage, size = cluster_size + 1),
        tooltip = sc1$mouseover,
        data_id = sc1$node,
        alpha = .5,
        data = sc1
      ) +
      xlab("Lineage and/or mutation") +
      ylab(varx) +
