...
message(paste("Starting scan", Sys.time()), "\n")
	...
	if (!ape::is.rooted(tre)) {
		if (!(root_on_tip %in% amd$sequence_name)) {
			...
			amd <- rbind(amd,
			             data.frame(sequence_name = root_on_tip,
	...
	tre <- ape::keep.tip(tre, intersect(tre$tip.label,
	                                    amd$sequence_name))
	tr2 <- tre
	if (!ape::is.rooted(tre)) {
		stopifnot(root_on_tip %in% tre$tip.label)
	...
	sts <- stats::setNames(amd$sample_time[match(tr2$tip.label,
	                                      amd$sequence_name)], tr2$tip.label)

	# root to tip
	ndel <- ape::node.depth.edgelength(tre)

	message(paste("Loaded tree & filtered by inclusion criteria",
	              Sys.time()))
	...
	{ # nolint
		...
		descendantSids <- lapply(1:(n + nnode),
	...
	.freq_figure <- function(.s1,
		...
		p <- ggplot2::ggplot() +
		  ggplot2::geom_point(ggplot2::aes(x = as.Date(lubridate::date_decimal(.data$time)),
		                                   y = .data$logodds,
		                                   size = n),
		                      data = estdf)  +
		  ...
		  ggplot2::geom_path(ggplot2::aes(x = as.Date(lubridate::date_decimal(.data$time)),
		                                  y = .data$estimated_logodds),
		                     data = estdf,
		                     color = "blue",
		                     size = 1) +
		  ggplot2::geom_ribbon(ggplot2::aes(x = as.Date(lubridate::date_decimal(.data$time)),
		                                    ymin = .data$lb,
		                                    ymax = .data$ub),
		                       data = estdf,
		                       alpha = .25)
	...
	.logistic_growth_stat <- function(u,
		...
		sta <- sts[ta]
		stu <- sts[tu]
		X <- data.frame(time = c(sta, stu),
		                type = c(rep("control", length(ta)),
		                         rep("clade", length(tu))))
		X <- stats::na.omit(X)
		...
		list(lgr = rv,
		     ...
		     relative_model_support = stats::setNames(exp(min(aic) - aic) / 2,
		                                       c("Logistic", "GAM")),
		     plot = .freq_figure(X)
		  )
	}
	...
	.cluster_tree <- function(tips) { # tr2, amd # tre?
		...
		nodedf <- data.frame(node = 1:(ape::Ntip(tr) + ape::Nnode(tr)),
		                     annot = annots,
		                     stringsAsFactors = FALSE)
		gtr1 <- gtr %<+% nodedf
		gtr1 <- gtr1 + ggplot2::geom_label(ggplot2::aes(x = .data$branch,
		                                                label = .data$annot,
		                                                size = 6))
	...
	.process.node <- function(u) {
		...
		# summary stat data
		utils::write.csv(data.frame(statistic = t(X[1, c("logistic_growth_rate",
